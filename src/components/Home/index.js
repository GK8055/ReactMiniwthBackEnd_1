...
class Home extends Component {
  ...
  onChangeRightArrowBtn = () => {
    const {navigatingIndex} = this.state
    if (navigatingIndex < 11) {
      this.setState(prev => ({navigatingIndex: prev.navigatingIndex + 1}))
    }
  }

  onChangeLeftArrowBtn = () => {
    const {navigatingIndex} = this.state
  ...
  onChangeEmojiBtnClk = id => {
    this.setState({activeEmojiId: id})
  }

  onChangeEmojisDropDownSelection = event => {
    this.setState({emojisListDropDownItem: event.target.value})
  }

  onChangeDaysDropDownSelection = event => {
    this.setState({daysListDropDownItem: event.target.value})
  }
    ...
    return (
      <EmojisCountContext.Consumer>
        {value => {
          ...
          // console.log('isMod', isModifiedList)
          ...
          const EMojisCount = renderSundayListEmojis(list)
          // console.log(EMojisCount)

          // DateModified...
          const onChangeDateBtnClk = id => {
            const emojiObj = emojisList.filter(
              each => each.id === activeEmojiId,
            )
            ...
            if (dateObj[0].emojiUrl === '') {
              ...
              if (dateObj[0].emojiName === emojisList[0].emojiName) {
                setVeryHappyEmojisCount(false)
              } else if (dateObj[0].emojiName === emojisList[1].emojiName) {
                setHappyEmojisCount(false)
              } else if (dateObj[0].emojiName === emojisList[2].emojiName) {
                setNeutralEmojisCount(false)
              } else if (dateObj[0].emojiName === emojisList[3].emojiName) {
                setSadEmojisCount(false)
              } else {
                setVerySadEmojisCount(false)
              }
                    ...
                    <ul className="emoji_selection_container">
                      {emojisList.map(each => (
                        <EmojiListView
                          ...
                          emojiBtnClk={this.onChangeEmojiBtnClk}
                        />
                      ))}
                    </ul>
                    ...
                    <div className="drop_down_container">
                      <div className="drop_downlist_container">
                        <select
                          ...
                          onChange={this.onChangeEmojisDropDownSelection}
                        ...
                        <select
                          ...
                          onChange={this.onChangeDaysDropDownSelection}
                          className="emoji_option_list_container"
                        >
                          {daysList.map(each => (
                            <option
